...
export function NotificationsBell({
  ...
  const refreshCount = useCallback(async () => {
    try {
      const response = await fetch("/api/opco/notifications/unread-count");
  ...
  useEffect(() => {
    ...
    window.addEventListener("opco-inbox-updated", handleUpdate);
    window.addEventListener("focus", handleUpdate);

    return () => {
      window.removeEventListener("opco-inbox-updated", handleUpdate);
