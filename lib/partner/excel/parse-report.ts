...
const COLUMN_ALIASES: Record<string, keyof Omit<ParsedReportLine, "lineNumber" | "sourceColumns">> = {
...
export async function parseReportWorkbook(
  ...
  const columnMap = new Map<number, keyof Omit<ParsedReportLine, "lineNumber" | "sourceColumns">>();
  ...
  if (columnMap.size === 0) {
    throw new ReportParseError(
      "No recognized columns found. Expected headers such as description, usage_amount, usage_usd.",
    );
  }
