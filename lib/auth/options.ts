 ...
 * Main portals (`/login`) and Admin (`/admin/login`) use separate cookie namespaces
 * so both sessions can coexist in the browser.
 */
...
function createAuthOptions(params: {
  cookieNamespace: AuthCookieNamespace;
  signInPage: string;
  defaultScope: AuthLoginScope;
}): NextAuthOptions {
  return {
    session: {
      strategy: "jwt",
    ...
    cookies: buildAuthCookies(
      secureAuthCookiesEnabled(),
      params.cookieNamespace,
    ),
    pages: {
      signIn: params.signInPage,
    },
    providers: [
      CredentialsProvider({
        name: "Credentials",
...
/** OpCo / Partner / Dizlee session (`/login`, `/api/auth`). */
export const authOptions: NextAuthOptions = createAuthOptions({
  cookieNamespace: "main",
  signInPage: "/login",
  defaultScope: "main",
});

/** Admin session (`/admin/login`, `/api/admin-auth`). */
export const adminAuthOptions: NextAuthOptions = createAuthOptions({
  cookieNamespace: "admin",
  signInPage: "/admin/login",
  defaultScope: "admin",
});

/** NextAuth basePath for Admin client signIn / SessionProvider. */
